<?php 
	...
	$wh_filde = "";
	if($S_NAME!=''){
		$wh_filde .= " AND lavel_name LIKE '%{$S_NAME}%'";
	}
	if($ACTVE_STATUS!=''){
		$wh_filde .= " AND active_status = '{$ACTVE_STATUS}'";
	}
	
	$sql = "SELECT * FROM lavel_user WHERE 1=1 {$wh_filde}";
        ...
        <div class="card">
          <div class="card-body">
            <form id="frm-search" action="" method="post">
				...
				<input type="hidden" id="lavel_id" name="lavel_id" value="">
				<input type="hidden" id="url_back" name="url_back" value="lavel_dis.php?settings=settings&manu_sub_id=<?php echo $_GET['manu_sub_id'];?>">
				<div class="row">
				...
				<div class="row">
					<div class="col-md-6">
					  <div class="form-group row" align="right">
						<label class="col-md-4 col-form-label">ระดับผู้ใช้งาน : </label>
						<div class="col-md-6">
				...
				<table class="table table-striped table-bordered">
				  <thead>
					<tr>
					  ...
					  <th style="text-align: center;"> ระดับผู้ใช้งาน </th>
				  ...
				  <tbody>
					...
					if($db->db_num_rows($query)){
					  ...
					  while($rec = $db->db_fetch_array($query)){
						$group ="<button type=\"button\" class=\"btn btn-outline-primary btn-sm\" onClick=\"GroupData('".$rec['lavel_id']."');\"><span class=\"mdi mdi-file-tree\" ></span></span> ตั้งค่าเมนู</button>";
						$edit ="<button type=\"button\" class=\"btn btn-outline-info btn-sm\" onClick=\"EditData('".$rec['lavel_id']."');\"><span class=\"mdi mdi-account-edit\" ></span></span>แก้ไข</button>";
						$del ="<button type=\"button\" class=\"btn btn-outline-danger btn-sm\" onClick=\"delData('".$rec['lavel_id']."');\"><span class=\"mdi mdi-delete\"></span>ลบ</button>";
					...
					?>
					  <tr id="tr_<?=$rec['lavel_id'];?>">
						<td style="text-align: center;"><?=$no;?></td>
						<td style="text-align: left;"><?=$rec['lavel_name'];?></td>
						<td style="text-align: center;"><?=($rec["active_status"]!='')?$active_name[$rec["active_status"]]:'';?></td>
						<td style="text-align: center;"><?=$group.' '.$edit.' '.$del;?></td>
					  </tr>
					<?php
						$no++;
						$group='';
...
function AddData(){
    $("#proc").val("add");
    $("#frm-search").attr("action","lavel_form.php").submit();
}
function GroupData(lavel_id){
    $("#proc").val("group");
    $("#lavel_id").val(lavel_id);
    $("#frm-search").attr("action","menu_group.php").submit();
}
function EditData(lavel_id){
    $("#proc").val("edit");
    $("#lavel_id").val(lavel_id);
    $("#frm-search").attr("action","lavel_form.php").submit();
}
function delData(lavel_id){
	Swal.fire({
	  ...
	  cancelButtonText: "ยกเลิก"
		...
		url = "process/lavel_proc.php";
		data = {proc:'del',lavel_id:lavel_id};
...
function stretchData(){
    $("#frm-search").attr("action","lavel_dis.php<?='?settings=settings&manu_sub_id='.$_GET['manu_sub_id'];?>").submit();
}
