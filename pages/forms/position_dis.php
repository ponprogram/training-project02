<?php 
	...
	$S_POS_NAME     = (isset($_POST['S_POS_NAME'])!='')?$_POST['S_POS_NAME']:'';
	$ACTVE_STATUS   = (isset($_POST['ACTVE_STATUS'])!='')?$_POST['ACTVE_STATUS']:'';
	
	$wh_filde = "";
	if($S_POS_NAME!=''){
		$wh_filde .= " AND pos_name LIKE '%{$S_POS_NAME}%'";
	}
	if($ACTVE_STATUS!=''){
...
<div class="main-panel">
  <div class="content-wrapper">
    <div class="page-header">
      <h1><i class="mdi mdi-settings icon-lg"></i> ตั้งค่าตำแหน่ง</h1>
    </div>
    <div class="row">
          ...
          <div class="card-body">
            <form id="frm-search" action="" method="post">
				<input type="hidden" id="manu_sub_id" name="manu_sub_id" value="<?php echo $_GET['manu_sub_id'];?>">
			  	...
			  	<input type="hidden" id="url_back" name="url_back" value="position_dis.php?settings=settings&manu_sub_id=<?php echo $_GET['manu_sub_id'];?>">
              <div class="row">
              ...
              <div class="row">
				<div class="col-md-6">
				  <div class="form-group row" align="right">
					<label class="col-md-4 col-form-label">ตำแหน่ง : </label>
					<div class="col-md-6">
					  <input type="text" class="form-control ClearData" id="S_POS_NAME" name="S_POS_NAME" value="<?=$S_POS_NAME;?>">
					</div>
				  </div>
				</div>
                <div class="col-md-6">
				...
				<table class="table table-striped table-bordered">
				  <thead>
					<tr>
					  ...
					  <th width='20%' style="text-align: center;"> แผนก </th>
				  ...
				  <tbody>
					...
					if($db->db_num_rows($query)){
					  ...
					  while($rec = $db->db_fetch_array($query)){
						$edit ="<button type=\"button\" class=\"btn btn-outline-info btn-sm\" onClick=\"EditData('".$rec['pos_id']."');\"><span class=\"mdi mdi-account-edit\" ></span></span>แก้ไข</button>";
						$del ="<button type=\"button\" class=\"btn btn-outline-danger btn-sm\" onClick=\"delData('".$rec['pos_id']."');\"><span class=\"mdi mdi-delete\"></span>ลบ</button>";
...
function AddData(){
    $("#proc").val("add");
    $("#frm-search").attr("action","position_form.php").submit();
}
function EditData(pos_id){
    $("#proc").val("edit");
    $("#pos_id").val(pos_id);
    $("#frm-search").attr("action","position_form.php").submit();
}
function delData(pos_id){
	Swal.fire({
	  ...
	  cancelButtonText: "ยกเลิก"
		...
		url = "process/position_proc.php";
		data = {proc:'del',pos_id:pos_id};
...
function stretchData(){
    $("#frm-search").attr("action","position_dis.php<?='?settings=settings&manu_sub_id='.$_GET['manu_sub_id'];?>").submit();
}
