<?php 
	...
	$S_PERFIX_NAME     = (isset($_POST['S_PERFIX_NAME'])!='')?$_POST['S_PERFIX_NAME']:'';
	$ACTVE_STATUS   = (isset($_POST['ACTVE_STATUS'])!='')?$_POST['ACTVE_STATUS']:'';
	
	$wh_filde = "";
	if($S_PERFIX_NAME!=''){
		$wh_filde .= " AND (perfix_name_th LIKE '%{$S_PERFIX_NAME}%' OR perfix_name_en LIKE '%{$S_PERFIX_NAME}%')";
	}
	if($ACTVE_STATUS!=''){
...
<div class="main-panel">
  <div class="content-wrapper">
    <div class="page-header">
      <h1><i class="mdi mdi-settings icon-lg"></i> ตั้งค่าแผนก</h1>
    </div>
    <div class="row">
          ...
          <div class="card-body">
            <form id="frm-search" action="" method="post">
				<input type="hidden" id="manu_sub_id" name="manu_sub_id" value="<?php echo $_GET['manu_sub_id'];?>">
			  	...
			  	<input type="hidden" id="perfix_id" name="perfix_id" value="">
			  	<input type="hidden" id="url_back" name="url_back" value="perfix_dis.php?settings=settings&manu_sub_id=<?php echo $_GET['manu_sub_id'];?>">
              <div class="row">
              ...
              <div class="row">
				<div class="col-md-6">
				  <div class="form-group row" align="right">
					<label class="col-md-4 col-form-label">คำนำหน้า : </label>
					<div class="col-md-6">
					  <input type="text" class="form-control ClearData" id="S_PERFIX_NAME" name="S_PERFIX_NAME" value="<?=$S_PERFIX_NAME;?>">
					</div>
				  </div>
				</div>
                <div class="col-md-6">
				...
				<table class="table table-striped table-bordered">
				  <thead>
					<tr>
					  ...
					  <th width='20%' style="text-align: center;"> คำนำหน้าภาษาไทย </th>
					  <th width='20%' style="text-align: center;"> คำนำหน้าภาษาอังกฤษ </th>
				  ...
				  <tbody>
					...
					if($db->db_num_rows($query)){
					 $no = $s_page+1;
					  while($rec = $db->db_fetch_array($query)){
						$edit ="<button type=\"button\" class=\"btn btn-outline-info btn-sm\" onClick=\"EditData('".$rec['perfix_id']."');\"><span class=\"mdi mdi-account-edit\" ></span></span>แก้ไข</button>";
						$del ="<button type=\"button\" class=\"btn btn-outline-danger btn-sm\" onClick=\"delData('".$rec['perfix_id']."');\"><span class=\"mdi mdi-delete\"></span>ลบ</button>";
					...
					?>
					  <tr id="tr_<?=$rec['perfix_id'];?>">
						<td style="text-align: center;"><?=$no;?></td>
						<td style="text-align: left;"><?=$rec["perfix_name_th"];?></td>
						<td style="text-align: left;"><?=$rec["perfix_name_en"];?></td>
...
function AddData(){
    $("#proc").val("add");
    $("#frm-search").attr("action","perfix_form.php").submit();
}
function EditData(perfix_id){
    $("#proc").val("edit");
    $("#perfix_id").val(perfix_id);
    $("#frm-search").attr("action","perfix_form.php").submit();
}
function delData(perfix_id){
	Swal.fire({
	  ...
	  cancelButtonText: "ยกเลิก"
		...
		url = "process/perfix_proc.php";
		data = {proc:'del',perfix_id:perfix_id};
		$.post(url,data,function(msg){
			...
			$('#tr_'+perfix_id).remove();
		});
	});
}
function stretchData(){
    $("#frm-search").attr("action","perfix_dis.php<?='?settings=settings&manu_sub_id='.$_GET['manu_sub_id'];?>").submit();
}
