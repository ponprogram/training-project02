<?php 
	...
	$sql = "SELECT
				mai.mai_id,
				IFNULL(mai.severity, 9) AS severity,
				mai.mai_date,
				mai.mai_detail,
				mai.user_id,
				mai.step_flow,
			...
			WHERE
				1 = 1 
                AND mai.step_flow IN (2,3,4,5) {$wh_filde}
            ORDER BY IFNULL(mai.severity, 9) ASC ";
			...
			<div class="card">
				<div class="card-body">
					<form id="frm-search" action="" method="post">
					  ...
					  <input type="hidden" id="url_back" name="url_back" value="flow_dis2.php<?=$Link;?>">
					  ...
					  <div class="row">
                        ...
                        <li class="nav-item">
                            ...
                            <li class="nav-item">
                                <a class="nav-link active" href="<?=$part;?>pages/flow/flow_dis2.php<?=$Link;?>">ระหว่างดำเนินการ</a>
                            </li> 
                            <li class="nav-item">
                                <a class="nav-link" href="<?=$part;?>pages/flow/flow_dis3.php<?=$Link;?>">ปิดงาน</a>
                            </li>
                        </ul>
                        <br>
                            ...
                            <tbody>
                                ...
                                if($db->db_num_rows($query)){
                                    ...
                                    while($rec = $db->db_fetch_array($query)){
                                        ...
                                        $sql_num = "SELECT count(*) AS num FROM maintenance_from WHERE mai_id = '".$rec['mai_id']."'";
                                        ...
                                        if($rec['step_flow']>=5){
                                            $end_job ="<button type=\"button\" class=\"btn btn-outline-primary btn-sm\" onClick=\"EndData('".$rec['mai_id']."');\"><span class=\"mdi mdi-calendar-check\"></span>ปิดงาน</button>";
                                        }
                                        // if($rec['step_flow']<=1){
                                        ...
                                        //     $next_step ="<button type=\"button\" class=\"btn btn-outline-warning btn-sm\" onClick=\"SendData('".$rec['mai_id']."');\"><span class=\"mdi mdi-send\"></span>ส่งมอบหมาย</button>";
                                        // }
                                        if($rec['step_flow']==2 && $rec_num['num']==0){
                                            $next_step ="<button type=\"button\" class=\"btn btn-outline-warning btn-sm\" onClick=\"ReturnData('".$rec['mai_id']."');\"><span class=\"mdi mdi-refresh\"></span>ดึงเรื่องกลับ</button>";
                                        }
                                        if($_SESSION['PER_LAVEL']==1){
...
function stretchData(){
    $("#frm-search").attr("action","flow_dis2.php<?=$Link;?>").submit();
}
