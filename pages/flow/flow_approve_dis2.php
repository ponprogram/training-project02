<?php 
	...
	$sql = "SELECT
				...
				mai.step_flow,
				mai.mai_number,
				CONCAT('[',mac.mac_number,'] ',mac.mac_name) AS mac_name,
				mac.mac_number,
				com.com_name,
				com.com_name_short,
				dep.dep_name 
			FROM
				maintenance_from maif
				INNER JOIN maintenance mai ON maif.mai_id = mai.mai_id
			...
			WHERE
				1 = 1 
				AND mai.step_flow > 4
                AND maif.frm_status IN ('N','Y','B')
				AND maif.user_id = '".$_SESSION['PER_ID']."' {$wh_filde}
			GROUP BY
				maif.frm_id,
				mai.mai_id,
				mai.severity,
			...
			<div class="card">
				<div class="card-body">
					<form id="frm-search" action="" method="post">
					  ...
					  <input type="hidden" id="url_back" name="url_back" value="flow_approve_dis2.php<?=$Link;?>">
					  <div class="row">
					...
					<ul class="nav nav-tabs">
                        <li class="nav-item">
                            <a class="nav-link" href="<?=$part;?>pages/flow/flow_approve_dis.php<?=$Link;?>">รอดำเนินการ</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link active" href="<?=$part;?>pages/flow/flow_approve_dis2.php<?=$Link;?>">ระหว่างดำเนินการ</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="<?=$part;?>pages/flow/flow_approve_dis3.php<?=$Link;?>">ปิดงาน</a>
                        </li>
                    </ul>
                    <br>
						  ...
						  <tbody>
							...
							if($db->db_num_rows($query)){
							  ...
							  while($rec = $db->db_fetch_array($query)){
								...
								// $assign ="<button type=\"button\" class=\"btn btn-outline-success btn-sm\" onClick=\"ApproveData('".$rec['mai_id']."','".$rec['frm_id']."');\"><span class=\"mdi mdi-folder-plus\"></span> แจ้งผลดำเนินการ</button>";
...
// function EditData(mai_id){
    ...
    // $("#frm-search").attr("action","flow_assign_form.php<?=$Link;?>").submit();
// }
function ApproveData(mai_id,frm_id){
    $("#proc").val("approve");
    $("#mai_id").val(mai_id);
    $("#frm_id").val(frm_id);
    $("#frm-search").attr("action","flow_approve_form.php<?=$Link;?>").submit();
}
...
function stretchData(){
    $("#frm-search").attr("action","flow_approve_dis2.php<?=$Link;?>").submit();
}
